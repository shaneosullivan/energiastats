  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      ...
      {view === 'weekly' && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-gray-500">Compare total weekly usage with the daily average for each week.</p>
            <div className="flex gap-2">
              ...
              >
                <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ backgroundColor: showTotal ? '#3b82f6' : '#d1d5db' }} />
                Week Total
              </button>
              <button
              ...
              >
                <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ backgroundColor: showDailyAvg ? '#93c5fd' : '#d1d5db' }} />
                Daily Average
              </button>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={weekly} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="label" tick={{ fontSize: 11 }} label={{ value: 'Week', position: 'insideBottom', offset: -2, style: { fontSize: 12, fill: '#9ca3af' } }} />
              <YAxis tick={{ fontSize: 11 }} label={{ value: 'kWh', angle: -90, position: 'insideLeft', style: { fontSize: 12 } }} />
              <Tooltip
                formatter={(value, name) => [`${value} kWh`, name]}
                contentStyle={{ fontSize: 12, borderRadius: 8 }}
              />
              {showTotal && <Bar dataKey="kwh" fill="#3b82f6" radius={[4, 4, 0, 0]} name="Week Total (kWh)" />}
              {showDailyAvg && <Bar dataKey="avgDaily" fill="#93c5fd" radius={[4, 4, 0, 0]} name="Daily Average (kWh)" />}
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {view === 'monthly' && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-gray-500">Compare total monthly usage with the daily average for each month.</p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowTotal(v => !v)}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border transition-colors ${showTotal ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={monthly} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="label" tick={{ fontSize: 12 }} label={{ value: 'Month', position: 'insideBottom', offset: -2, style: { fontSize: 12, fill: '#9ca3af' } }} />
              <YAxis tick={{ fontSize: 11 }} label={{ value: 'kWh', angle: -90, position: 'insideLeft', style: { fontSize: 12 } }} />
              <Tooltip
                formatter={(value, name) => [`${value} kWh`, name]}
                contentStyle={{ fontSize: 12, borderRadius: 8 }}
              />
              {showTotal && <Bar dataKey="kwh" fill="#3b82f6" radius={[4, 4, 0, 0]} name="Month Total (kWh)" />}
              {showDailyAvg && <Bar dataKey="avgDaily" fill="#93c5fd" radius={[4, 4, 0, 0]} name="Daily Average (kWh)" />}
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      {view === 'hourly' && (
        <ResponsiveContainer width="100%" height={350}>
          <AreaChart data={hourly} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            <defs>
            ...
            <YAxis tick={{ fontSize: 11 }} label={{ value: 'Avg kWh (per 30min)', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }} />
            <Tooltip
              formatter={(value) => [`${value} kWh`, 'Avg per 30-min slot']}
              contentStyle={{ fontSize: 12, borderRadius: 8 }}
            />
            <Area type="monotone" dataKey="avgKwh" stroke="#3b82f6" fill="url(#hourGrad)" strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      )}
