...
function simulateDay(
  ...
  if (currentBattery.hasBattery && currentBattery.capacityKwh > 0) {
    const usable =
      currentBattery.capacityKwh * (currentBattery.usablePercent / 100);
    const chargeHours = currentBattery.autoDetectCheapest
      ? findCheapestPeriod(currentTariff, dayOfWeek)
      : {
          startHour: currentBattery.chargeStartHour,
          startMinute: currentBattery.chargeStartMinute,
          endHour: currentBattery.chargeEndHour,
          endMinute: currentBattery.chargeEndMinute,
        };
    ...
    for (const r of readings) {
      ...
      if (
        isInTimeRange(
          h,
          m,
          currentEV.chargingStartHour,
          currentEV.chargingStartMinute,
          currentEV.chargingEndHour,
          currentEV.chargingEndMinute,
        )
      ) {
  ...
  if (simBattery.hasBattery && simBattery.capacityKwh > 0) {
    ...
    const chargeHours = simBattery.autoDetectCheapest
      ...
      : {
          startHour: simBattery.chargeStartHour,
          startMinute: simBattery.chargeStartMinute,
          endHour: simBattery.chargeEndHour,
          endMinute: simBattery.chargeEndMinute,
        };
